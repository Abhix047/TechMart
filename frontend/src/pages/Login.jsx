...
const Login = ({ onSwitch, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onClose(); // Modal band karo login ke baad
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-5">
        ...
        {/* Email Input (Glass Outline Style) */}
        ...
        {/* Password Input (Glass Outline Style) */}
        <div className="relative flex items-center group">
        ...
        {/* Remember Me Checkbox (Optional, matching image) */}
        <div className="flex items-center gap-2 px-1">
            <input type="checkbox" id="remember" className="w-4 h-4 accent-green-500 rounded cursor-pointer" />
            <label htmlFor="remember" className="text-white/90 text-xs cursor-pointer font-medium">
              Remember me
            </label>
        </div>

        {/* Action Button (Green Gradient like the image) */}
        <button 
          disabled={loading} 
