...
import { Mail, Lock, User, X, ArrowRight, Eye, EyeOff, Star, Zap, Gift } from "lucide-react";
      ...
      <div className="hidden md:flex flex-col justify-between bg-[#0a0a0a] relative overflow-hidden p-9 shrink-0 w-[300px]">
        ...
        <div className="flex-1 flex flex-col justify-center py-7">
          <div className="flex flex-col gap-1.5 mb-6">
            <WordReveal words={[
              { text: "Join the", italic: false, faded: false, delay: 0.15 },
              { text: "future",   italic: true,  faded: true,  delay: 0.27 },
              { text: "of",       italic: false, faded: false, delay: 0.38 },
              { text: "tech.",    italic: true,  faded: true,  delay: 0.50 },
            ]} />
          </div>

          <motion.div className="h-px bg-white/[0.18] mb-5 origin-left"
            initial={{ scaleX: 0 }} animate={{ scaleX: 1 }}
            transition={{ duration: 0.9, delay: 0.66, ease: [0.22, 1, 0.36, 1] }}
          />

          <motion.p className="font-[family-name:'DM_Sans',sans-serif] text-[12px] font-light text-white/38 leading-[1.75] mb-5"
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.82, ease: [0.22, 1, 0.36, 1] }}
          >
            Create your account and unlock<br />exclusive member benefits.
          </motion.p>

          {[
            { Icon: Star, text: "Exclusive member deals",  d: 0.92 },
            { Icon: Zap,  text: "Early access to launches", d: 1.02 },
            { Icon: Gift, text: "Special birthday offers",  d: 1.12 },
          ].map(({ Icon, text, d }) => (
        ...
        <motion.p className="font-[family-name:'DM_Sans',sans-serif] text-[9px] text-white/[0.16]"
      ...
      <div className="flex-1 bg-white flex flex-col relative">
        ...
        <div className="flex-1 px-8 pt-9 pb-5">
          ...
          >
            ...
            <h2 className="font-[family-name:'Cormorant_Garamond',serif] font-semibold text-[#0a0a0a] leading-none m-0 text-[clamp(26px,3.5vw,34px)]">
              Create <em className="italic font-[500]">Account</em>
            </h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            {/* Name */}
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.14, ease: [0.22, 1, 0.36, 1] }}>
              <label className="block font-[family-name:'DM_Sans',sans-serif] text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50 mb-1.5">Full Name</label>
              <InputField icon={User} type="text" name="name" placeholder="Arjun Sharma" value={form.name} onChange={handleChange} />
            </motion.div>

            {/* Email */}
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.20, ease: [0.22, 1, 0.36, 1] }}>
              <label className="block font-[family-name:'DM_Sans',sans-serif] text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50 mb-1.5">Email Address</label>
              <InputField icon={Mail} type="email" name="email" placeholder="your@email.com" value={form.email} onChange={handleChange} />
            </motion.div>

            {/* Password */}
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.26, ease: [0.22, 1, 0.36, 1] }}>
              <label className="block font-[family-name:'DM_Sans',sans-serif] text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50 mb-1.5">Password</label>
              <InputField icon={Lock} type="password" name="password" placeholder="Min. 8 characters" value={form.password} onChange={handleChange} showToggle onToggle={() => setShowPass(v => !v)} showPass={showPass} />
            </motion.div>

            {/* Terms */}
            <motion.p className="font-[family-name:'DM_Sans',sans-serif] text-[11px] text-black/40 leading-relaxed"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.35, delay: 0.32 }}
            >
              By signing up, you agree to our{" "}
              <span className="text-[#0a0a0a] font-medium cursor-pointer underline underline-offset-[2px]">Terms</span>
              {" "}and{" "}
              <span className="text-[#0a0a0a] font-medium cursor-pointer underline underline-offset-[2px]">Privacy Policy</span>.
            </motion.p>
        ...
        <div className="flex items-center justify-center gap-1.5 py-2.5 border-t border-black/[0.06]">
          ...
          <span className="font-[family-name:'DM_Sans',sans-serif] text-[10px] text-black/24">SSL encrypted · 100% secure</span>
        </div>
      </div>
    </motion.div>
  );
